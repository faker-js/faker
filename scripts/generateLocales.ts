 ...
 * TODO @Shinigami92 2022-08-19: can the pre-bundling of locales completely moved to this script?
...
const pathDocs = resolve(pathRoot, 'docs');
const pathSrc = resolve(pathRoot, 'src');
const pathLocale = resolve(pathSrc, 'locale');
const pathLocales = resolve(pathSrc, 'locales');
const pathLocalesIndex = resolve(pathSrc, 'all-locales.ts');
const pathDocsApiLocalization = resolve(pathDocs, 'api', 'localization.md');
...
function generateLocaleFile(locale: string): void {
  let content = `
      ...
      ${locale !== 'en' ? "import en from '../locales/en.json';" : ''}
...
function _tryLoadLocalesMainIndexFile(pathModules: string): LocaleDefinition {
...
function _generateLocalesIndexFile(
  path: string,
  name: string,
  type: string,
  depth: number,
...
function _generateRecursiveModuleIndexes(
  path: string,
  name: string,
  definition: string,
  depth: number,
...
for (const locale of locales) {
  ...
  localeIndexImports += `import ${locale} from './locales/${locale}.json';\n`;
