...
export async function writeApiPagesIndex(pages: Page[]): Promise<void> {
  const sidebarGroup = groupBy(pages, (page) => page.apiCategory);
  const pagesSidebarCategory = Object.entries(sidebarGroup).flatMap(
    ([category, items]): DefaultTheme.SidebarItem[] =>
      category ? [{ text: category, items }] : items
  );

  // Write api-pages.ts
  console.log('Updating api-pages.ts');
  pagesSidebarCategory.splice(0, 0, { text: 'Overview', link: '/api/' });
  let apiPagesContent = `
    // This file is automatically generated.
    // Run '${scriptCommand}' to update
    export const apiPages = ${JSON.stringify(pagesSidebarCategory)};
    `.replace(/\n +/, '\n');
