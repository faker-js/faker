 ...
 * @param apiCategory The category of the module.
 */
...
export async function writeApiPagesIndex(pages: Page[]): Promise<void> {
  const API_category = new Map();
  const pages_ = pages.reduce((memorize: DefaultTheme.SidebarItem[], page) => {
    if (page.apiCategory) {
      const category = API_category.get(page.apiCategory);
      API_category.set(
        page.apiCategory,
    ...
    return [...memorize, page];
  }, []);

  for (const [category, value] of API_category) {
    pages_.push({
      text: category,
      items: value,
    });
  ...
  pages_.splice(0, 0, { text: 'Overview', link: '/api/' });
  let apiPagesContent = `
    // This file is automatically generated.
    // Run '${scriptCommand}' to update
    export const apiPages = ${JSON.stringify(pages_)};
    `.replace(/\n +/, '\n');
